import { faCheck, faExchangeAlt, faWallet } from "@fortawesome/free-solid-svg-icons";
...
export default function RechargeChip (){
    ...
    const handleInitiatePayment = async () => {
        ...
        const options = {
          ...
          prefill: {
            name: user?.userName, // Ideally, fetch user's real name from your user management system
            email: user?.email,
            contact: "9999999999", // Ideally, fetch user's contact from your system
          },
          theme: {
            color: "#3399cc",
          },
    ...
    return(
        <>
        <div className="flex bg-blue-400 text-white justify-between p-4">
          <div>
               <span className="font-serif text-xl">Recharge-Wallet</span>
          </div>
          <div>
         ...
         <div className="flex justify-center mt-10">
             <div className="flex items-center gap-4 text-xl font-bold">
                 <span>Available Balance:  ₹ {user?.balance / 100}</span>
              </div> 
         </div>
          
          <div className="justify-center flex mt-10 items-center">
               <FontAwesomeIcon icon={faWallet} className="text-2xl text-gray-200 absolute left-96"/>
               <input type="number" className="bg-gray-400 outline-none text-white w-[600px] p-3 text-center" value={amount} placeholder="Select a amount to recharge" />
          </div>

          <div className="flex justify-center gap-3 mt-3">
             {chips.map((i) => (
                <div key={i.id} className="h-10 w-10 cursor-pointer rounded-full" onClick={() => setAmount(i.value)}>
                    <img src={i.url}/>
                </div>
             ))}    
          </div>
          <div className="mt-10 m-5">
              <span className="font-serif text-lg">Available Payment Methods</span>
              <div>
                  {
                    paymentmethods.map((i) => (
                        <div key={i.id} className="flex justify-between cursor-pointer">
                           
                            <div onClick={() => setSelectedPaymentMode(i.id)}>
                                  {i.logo()}
                             </div>
                             <div>
                                 {
                                    selectedpaymentmode && <FontAwesomeIcon icon={faCheck} className="absolute"/>
                                 }
                             </div>
                        </div>
                    ))
              ...
              <div className="justify-center flex mt-10">
                  <button className="bg-blue-400 text-white p-3  rounded-md" onClick={handleInitiatePayment}>Recharge Chip</button>
              </div>
          </div>
        </>
    )
